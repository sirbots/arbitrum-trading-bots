...
SUSHI_POOL_CONTRACT_ADDRESS = "0x905dfCD5649217c42684f23958568e533C711Aa3" # WETH-USDC pool
...
def main():
    ...
    sushi_lp = contract_load(
        SUSHI_POOL_CONTRACT_ADDRESS, "Sushi LP: WETH-USDC"
    )
    ...
    while True:
        ...
        # BUY_WETH_THRESHOLD = Decimal("0.10")
        # SELL_WETH_THRESHOLD = Decimal("-0.50")
        ...
        if balance_refresh:
            ...
            print("\nAccount Balance:")
            print(
                f"• Token #1: {round(weth['balance']/(10**weth['decimals']), 3)} {weth['symbol']} ({weth['name']})"
            )
            print(
                f"• Token #2: {int(usdc['balance']/(10**usdc['decimals']))} {usdc['symbol']} ({usdc['name']})"
            )
            print()
            print(f"Current WETH market price: ${round(weth_market_price, 3)}")
        ...
        if usdc["balance"]:
            ...
            try:
                # token0 (x) is WETH
                # token1 (y) is USDC
                x0, y0 = sushi_lp.getReserves.call()[0:2]
            except:
                continue
...
def log_transaction():
    ...
    try:
        ...
        # Write the CSV file
        filename = 'logs/arbitrum_bot.csv'
        row = [
            csv_date,
            csv_time,
            csv_account_name,
