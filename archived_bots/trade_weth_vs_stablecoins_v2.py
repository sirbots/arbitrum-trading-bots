...
def main():
    ...
    while True:
        ...
        try: 
            ...
            # oh! because if you incorporate slippage, then you can know the exact level where it's profitable, and do trades at much lower net profits!!!

            # how to better calculate the arb profit...
                                # Raw trade: (300 / 1223.40 * 1233.08) - 300    = $2.373
                                # Pool fees: $300 * 0.003 * 2                   = $1.80
                                # Gas fees:  $0.30 * 2                          = $0.60
                                # Net:                                          = $(0.027)
                                #
                                # Max slippage: 0.017 * 300 * 2                 = $10.20
                                # Net (with MAX slippage)                       = ($10.227)
                                
                                # Raw trade: (500 / 1223.40 * 1233.08) - 500    = $3.9561876737
                                # Pool fees: $500 * 0.003 * 2                   = $3.00
                                # Gas fees:  $0.30 * 2                          = $0.60
                                # Net:                                          = $0.3561876737
                                #
                                # Max slippage: 0.017 * 500 * 2                 = $17
                                # Net (with MAX slippage)                       = ($16.643)

            # OK, what if we only did USDC and MIM pools, so as to eliminate the slippage problem? Let's try lowering slippage down to 0.001 again
                                # Raw trade: (300 / 1219.56 * 1233.63) - 300    = $3.4610843255
                                # Pool fees: $300 * 0.003 * 2                   = $1.80
                                # Gas fees:  $0.30 * 2                          = $0.60
                                # Net:                                          = $1.0610843255
                                #
                                # Max slippage: 0.001 * 300 * 2                 = $0.60
                                # Net (with MAX slippage)                       = $0.4610843255
            ...
            if last_arb_profit != current_arb_profit:
                print(
                    f"{datetime.datetime.now().strftime('%D')} | {datetime.datetime.now().strftime('%H:%M:%S')} | {chain.height} | USDC: ${usdc['pool_price_of_weth']:.2f} | USDT: ${usdt['pool_price_of_weth']:.2f} | MIM: ${mim['pool_price_of_weth']:.2f} | DAI: ${dai['pool_price_of_weth']:.2f} | Net: ${(current_arb_profit):.2f} "
                )
